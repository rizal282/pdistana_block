<?php
    ...
    if(isset($_POST["suratjalan_tr"])){
        ...
        $sqlgetdetailbeli = mysqli_query($koneksi, "select * from barang_rusak inner join stock_barang on barang_rusak.kode_brg = stock_barang.kode_barang where barang_rusak.no_transaksi = '".$suratjalan_tr."'");

        // $sqlgetsuratjalan = mysqli_query($koneksi, "select * from surat_jalan_feedback inner join karyawan on surat_jalan_feedback.id_supir = karyawan.id_kry where surat_jalan_feedback.no_transaksi = '".$suratjalan_tr."'");
        $sqlgetsuratjalan = mysqli_query($koneksi, "select * from surat_jalan_feedback where no_transaksi = '".$suratjalan_tr."'");
        $rowsuratjalan = mysqli_fetch_array($sqlgetsuratjalan);
    }

    // memanggil library Fthis
    ...
    $pdf->AddPage();
     ...
     $pdf->Image('img/logopdistana.png',10,10,30);
            ...
            // Move to the right
            $pdf->Cell(20,5,'ISTANA BLOCK',0,0,'C');

            $pdf->Cell(180,5,'Sawah Kulon Pasawahan Kab.Purwakarta Jawa Barat 41172                         ',0,0,'C');
            ...
            $pdf->Cell(180,5,'Phone : 0812 8177 7660 - 0878 8177 7660                           ',0,0,'C');
            ...
            while($dataorderbeli = mysqli_fetch_array($sqlgetdetailbeli)){
                $pdf->Cell(150,5,$dataorderbeli["nama_barang"],0,0,'C');
                $pdf->Cell(50,5,$dataorderbeli["satuan_brg"],0,0,'C');
                $pdf->Cell(130,5,$dataorderbeli["qty_barang"],0,1,'C');
            }

            // $totalseluruh = array_sum($nominalbayar);
           ...
           // Line break
            ...
            $pdf->Cell(82,7,ucwords($rowsuratjalan["pembuat_surat_jln"]),0,1,'C');
