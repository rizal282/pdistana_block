<?php
  ...
  if (isset($_POST["tanggalAkhir"])) {
    ...
    $getfilterbayartunai = mysqli_query($koneksi, "select * from order_bahanbaku inner join total_bayar_bahanbaku on order_bahanbaku.no_trbahanbaku = total_bayar_bahanbaku.no_trbahanbaku where total_bayar_bahanbaku.sts_lunas = 'Sudah Lunas' and order_bahanbaku.tgl_pembelian between '".$tanggalAwal."' and '".$tanggalAkhir."'");
    $hitungdatafilterbayartunai = mysqli_num_rows($getfilterbayartunai);

    if($hitungdatafilterbayartunai == 0){
      echo '<tr>
        <td colspan="9">Tidak Ada Data Belanja Bahan Baku Lunas</td>
      </tr>';
    }else{
      $nomor = 1;
