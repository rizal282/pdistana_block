<?php
	...
	class buatPdf extends FPDF{
		function Header(){
			...
			$this->Cell(297,7,'LAPORAN KAS BESAR',0,1,'C');
			...
			$this->Cell(55,6,'Tanggal',1,0);
			$this->Cell(52,6,'No Transaksi',1,0);
	...
	while ($row = mysqli_fetch_array($pemasukan)){
	    ...
	    $pdf->Cell(55,6,date("d M Y", strtotime($row['tgl_bayar'])),1,0);
	    $pdf->Cell(52,6,$row['no_transaksi'],1,0);
	...
	$pdf->Output("I","LAPORAN KAS BESAR ISTANA BLOCK ".date("YmdHis").".pdf");
	ob_end_flush();
?>
